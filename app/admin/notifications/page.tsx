    ...
    <div className="space-y-5 max-w-3xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Real-time alerts for member submitted payment slips, settled receipts, and system events.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/admin/receipts">
            <Button variant="outline" size="sm" className="text-xs font-semibold border-slate-200 cursor-pointer">
              <Receipt className="h-4 w-4 text-emerald-700 mr-1.5" /> Review Slips
            </Button>
          </Link>
          {unreadCount > 0 && (
            <Button
      ...
      {data?.data.length === 0 && !isLoading && (
        <div className="bg-white p-8 text-center rounded-xl border border-slate-200">
          ...
          <p className="text-slate-400 text-xs mt-0.5">When members submit receipt slips or make payments, alerts will appear here automatically.</p>
        </div>
      )}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {isReceiptAlert && (
                        ...
                        >
                          Verify in Receipts &amp; Slips <ExternalLink className="h-3 w-3" />
                        </Link>
                      )}
                    </div>
                  </div>
